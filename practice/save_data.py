...
for i in range(len(web_addresses)):
    ...
    elif new_record.id == 'EH':
        eh_records.append(new_record)
    elif new_record.id == 'SH':
        sh_records.append(new_record)
    elif new_record.id == 'NW':
        nw_records.append(new_record)
    elif new_record.id == 'SU':
...
pickle.dump(eh_records, out_file)
out_file.close()

out_file_wh = open('record_list_wh_file', 'wb')
pickle.dump(wh_records, out_file)
out_file.close()

out_file_sh = open('record_list_sh_file', 'wb')
pickle.dump(sh_records, out_file)
out_file.close()

out_file_su = open('record_list_su_file', 'wb')
pickle.dump(su_records, out_file)
out_file.close()
